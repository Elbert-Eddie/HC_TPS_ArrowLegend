...
public class HpBarControl : MonoBehaviour
{
    ...
    /// <returns></returns>
    public IEnumerator ShowDamage(float damage)    // 協程方式：顯示傷害值 - 接收傷害
    {
        Vector3 posOriginal = textDamage.transform.position;            // 取得原始座標
        textDamage.text = "-" + damage;   // 更新傷害值 = 接收傷害
     for (int i = 0; i < 40; i++)   // 迴圈
        {
            textDamage.transform.position += new Vector3(0, 0.05f, 0);  // 傷害值往上移動 (transform.position.y += 值)
            yield return new WaitForSeconds(0.005f);                    // 等待
        }
